...
for(i in parelles){
    ...
    n1=strsplit(i[[1]][[1]],"Adria")[[1]][1]
  	...
  	print(dir3)
    ptm=proc.time()
    mat=try(load(paste("Results/Alin",n1,n2,".RData",sep="")),TRUE)
    if(class(mat)=="try-error"){print(mat)}
  	if(class(mat)!="try-error"){
    	print("OK")
#      TTT[n1,n2]=paste(proc.time()-ptm, collapse=" ")
 #     print(TTT[n1,n2])
   # 	save(matriu,file=paste("Files/matrius/",n1,n2,".RData",sep=""))
